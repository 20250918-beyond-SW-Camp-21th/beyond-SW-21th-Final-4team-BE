...
class ScoreDto(BaseModel):
    ...
    score: int = Field(description="해당 항목의 평가 점수 (1~5점)")
    ...
    try:
        ...
        try:
            with conn.cursor() as cursor:
                # 기업이 프리랜서에게 남긴 리뷰 조회
                sql = "SELECT description FROM employer_freelancer_reviews WHERE freelancer_id = %s AND status = 'ACTIVE' LIMIT 50"
        ...
        아래는 특정 프리랜서가 과거 클라이언트들(기업)로부터 받은 리뷰 내용입니다.
        이 리뷰들을 바탕으로 종합적인 평판을 요약해주고, 강점/약점과 세부 역량 점수들(1~5점 기준)을 도출해주세요.
        ...
        result = await structured_llm.ainvoke(prompt.format(reviews=truncated_reviews))
        return result
