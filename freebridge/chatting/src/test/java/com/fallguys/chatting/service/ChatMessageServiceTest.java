...
class ChatMessageServiceTest {
    ...
    void sendMessage_Success() {
        ...
        // when
        ChatMessageResponse response = chatMessageService.sendMessage("room1", "e1", "안녕하세요", MessageType.TEXT, null);
    ...
    void getPreviousMessages_Success() {
        ...
        com.fallguys.chatting.dto.CursorPageResponse<ChatMessageResponse> response = chatMessageService
                .getPreviousMessages("room1", null, 20);

        // then
        assertThat(response.getItems()).hasSize(2);
        assertThat(response.getItems().get(0).getContent()).isEqualTo("M2");
        assertThat(response.getNextCursor()).isEqualTo("msg1"); // msg1의 ID나 시간이어야 함. 도메인의 createdAt이 null이므로 임시로 ID 비교는
                                                                // 로직에 따라 다름.
    }
}
