...
public class EmployerProjectController {
    ...
    @GetMapping("/stats")
    public ApiResponse<EmployerProjectStatsResponseDto> getProjectStats(@AuthenticationPrincipal String userId) {
        return ApiResponse.ok(null);
    }

    @Operation(summary = "내 프로젝트 목록 조회", description = "마이페이지에서 노출할 내 프로젝트 리스트를 조회합니다.")
    @GetMapping
    public ApiResponse<List<EmployerProjectListResponseDto>> getMyProjects(@AuthenticationPrincipal String userId,
    ...
    public ApiResponse<List<EmployerApplicantStatusResponseDto>> getApplicantStatus(@AuthenticationPrincipal String userId,
